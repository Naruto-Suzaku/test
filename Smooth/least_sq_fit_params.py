...
class free_free(object):
    ...
    def __init__(self, v, nside, index_type, dist,emi_form,I_E_form):
    ...
    def produce_xyz(self):
        #v in Mhz
        result = smooth(self.nside,self.v, self.index_type,self.I_E_form).add_5()
        return result

    def diffuse_raw(self):
        diffuse_x = smooth(self.nside, self.v, self.index_type,self.I_E_form).produce_data()
        return diffuse_x
 
    def fun_for_curvefit(self, p, x):
        #produce for input for curve_fit
        
        A_v, R_0, alpha, R_1, beta, Z_0, gamma = p
        result = []
        for l,b in x:
    ...
    def err(self,p,x,y):
        return (self.fun_for_curvefit(p,x) - y)/y

    def curve_fit(self,form='CRs'):
        if form == 'CRs':
            #R_0, alpha, a,B,c,D,g
            #A_v, R_0, alpha, R_1, beta, Z_0, gamma
            p0 = [1e7,8.5,2,4,2,3,1]

        func = self.err
        xyz = self.produce_xyz()
        print 'xyz.shape',xyz.shape
        params,guess = leastsq(func, p0, args=(xyz[:,:2], xyz[:,2]))
        print 'params,guess',params,guess
    ...
    def delta_m(self):
        ...
        for pix_number in range(0,hp.nside2npix(nside)):
            ...
            a = time.time()
            pix_value = self.model_m2(l,b,abcz0) + self.I_E(self.v)
        ...
        delt_m = diffuse_raw + self.I_E(self.v) - m 
