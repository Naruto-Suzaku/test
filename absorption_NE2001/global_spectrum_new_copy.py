    ...
    def mpi(self):
        ...
        for pix_number in local_range:
            ...
            if self.test == True:
                ...
                # it is time consuming to calculate the distance
                distance = 1.
                l, b = hp.pix2ang(self.nside, pix_number, nest = False, lonlat = True)
        ...
        if rank == 0:
            if self.test == True:
                ...
                with h5py.File('./output/' + 'exp'+str(self.v)+'Mhz_delt_m_and_unabsorb_and_delt_m_percentage.hdf5','r') as f:
                    print f.keys()
                    unabsorb = f['integrated_temperature_total_m'][:]
                    diffuse_raw = f['diffuse_raw'][:]
                #read abosrb + I_E*exp(-tao(50))
                #with h5py.File('./input/'+'exp'+str(int(self.v))+'MHz_global_spectrum.hdf5','r') as g:
                #    absorb = g['result_absorb'][:,1]
                ...
                with h5py.File('./output/' + str(self.emi_form)+str(self.v)+'MHz_global_spectrum.hdf5','w') as h:
